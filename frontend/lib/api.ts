...
export const CreaturesAPI = {
  ...
  resolveCombat: async (id: number, damageReceived: number, won: boolean): Promise<Creature> => {
    const response = await api.post<Creature>(`/creatures/${id}/combat-result`, null, {
      params: { damageReceived, won },
    })
    return response.data
  },
}
...
export const AdminAPI = {
  ...
  getUserCreatures: async (ownerId: number): Promise<Creature[]> => {
    try {
      const response = await api.get<Creature[]>(`/admin/users/${ownerId}/creatures`)
      return Array.isArray(response.data) ? response.data : []
    } catch (error: any) {
      console.error("Error fetching user creatures:", error.message)
      return []
    }
  },
